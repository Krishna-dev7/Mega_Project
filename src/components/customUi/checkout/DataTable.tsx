...
export function DataTable<TData, TValue>({
	...
	return (
      ...
      sm:text-sm max-w-4xl mx-auto sm:px-10 py-8">
			...
			{table.getRowCount() !== 0 && (
				<div className="delete-section flex space-x-2 justify-end">
					<ConfirmDialog
						title="Warning 🤚"
                 ...
                 ${
					...
					<CheckoutButton
						...
						items={table.getRowModel().rows.map((row) => ({
							...
							price: (row.original as cartType).product
								.price,
							quantity: (row.original as cartType).quantity,
							image: (row.original as cartType).product
								.images[0].url,
						}))}
					/>
				</div>
