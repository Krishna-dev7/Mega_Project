...
const OrderPage: React.FC = () => {
  ...
  const userId: (string | undefined) = useAppSelector(store => store.auth?.data?._id.toString())
  ...
  useEffect(() => {
    try {
      ...
      (async function () {
        ...
        const orders = res.data as (IOrder & {userId: UserSchema})[];
        const filteredOrders = orders.filter(item => item.userId._id.toString() === userId);
  ...
  justify-start text-xs mt-20 gap-6 ">
